...
def test_msix_manifesto_acompanha_o_produto_e_seus_logotipos():
    """O empacotador MSIX já apontou para o produto errado; isto trava a volta."""
    import empacotar_msix
    from provas.recursos import PRODUCT_NAME

    xml = empacotar_msix.manifesto()

    assert rf'Executable="{PRODUCT_NAME}\{PRODUCT_NAME}.exe"' in xml
    assert f"<DisplayName>{PRODUCT_NAME}</DisplayName>" in xml
    assert empacotar_msix.NOME == PRODUCT_NAME
    for arquivo in empacotar_msix.LOGOTIPOS:
        assert rf"Assets\{arquivo}" in xml, f"logotipo gerado e nunca referenciado: {arquivo}"
    # A Store recusa versão cujo último número não seja 0.
    assert empacotar_msix.VERSAO.endswith(".0")
